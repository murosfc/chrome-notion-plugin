...
class PopupManager {
  ...
  initializeElements() {
    ...
    this.defaultBranchInput = document.getElementById("defaultBranchInput");
  ...
  setupEventListeners() {
    ...
    this.defaultBranchInput.addEventListener("input", () => {
      this.clearFieldStyles(this.defaultBranchInput);
    });
  }
    ...
    try {
      ...
      if (response.success) {
        ...
        if (config.defaultBaseBranch) {
          this.defaultBranchInput.value = config.defaultBaseBranch;
        }
  ...
  async saveConfiguration() {
    ...
    const defaultBaseBranch = this.defaultBranchInput.value.trim();
    ...
    try {
      const config = { geminiApiKey: apiKey, projectPath };
  ...
  async testConnections() {
    console.log("🔍 testConnections called");
      ...
      if (response.success) {
        ...
        if (results.geminiAPI.status === "success") {
          ...
          if (this.isApiError(results.geminiAPI.message)) {
            ...
            // Show error with API key reconfiguration options
            this.showError(
              "API key issue detected - please reconfigure. " +
                results.geminiAPI.message
            );
          }
        }
  ...
  showError(message) {
    ...
    if (this.isApiError(message)) {
      ...
      // Show update API key button for all API errors
      this.errorActions.innerHTML = `
        <div style="margin-bottom: 12px;">
          <strong>API key issue detected - please reconfigure</strong>
        </div>
        <button id="overwriteApiKeyBtn" class="btn btn-primary btn-small" style="margin-right: 8px;">
          🔑 Click here to overwrite your API Key
        </button>
        <button id="testConnectionAgainBtn" class="btn btn-secondary btn-small">
          🔄 Retry Connection
        </button>
      `;
      ...
      setTimeout(() => {
        const overwriteApiKeyBtn =
          document.getElementById("overwriteApiKeyBtn");
        const testConnectionAgainBtn = document.getElementById(
          "testConnectionAgainBtn"
        );

        if (overwriteApiKeyBtn) {
          overwriteApiKeyBtn.addEventListener("click", () => {
            this.showApiKeyOverwriteDialog();
          });
        }

        if (testConnectionAgainBtn) {
          testConnectionAgainBtn.addEventListener("click", () => {
            this.testConnections();
          });
        }
      }, 100);
  ...
  showApiKeyOverwriteDialog() {
    // Create a modal-like dialog for API key input
    const dialogHtml = `
      <div id="apiKeyOverwriteModal" style="
        position: fixed;
        top: 0;
        left: 0;
        width: 100%;
      ...
      ">
        ...
        ">
          <h3 style="margin-top: 0; color: #333;">🔑 Overwrite API Key</h3>
          <p style="color: #666; margin-bottom: 15px;">
            Enter your new Gemini API Key to replace the current one:
          </p>
          ...
          <div style="margin-bottom: 15px;">
            <label style="display: flex; align-items: center; font-size: 12px; color: #666;">
              <input type="checkbox" id="showNewApiKey" style="margin-right: 5px;">
              Show API key
            </label>
          </div>
          <div style="display: flex; gap: 10px; justify-content: flex-end;">
            <button id="cancelOverwriteBtn" style="
              padding: 8px 16px;
              border: 1px solid #ddd;
              background: white;
              border-radius: 4px;
              cursor: pointer;
            ">Cancel</button>
            <button id="confirmOverwriteBtn" style="
              padding: 8px 16px;
              border: none;
              background: #007cba;
              color: white;
    ...
    const showNewApiKey = document.getElementById("showNewApiKey");
    ...
    showNewApiKey.addEventListener("change", () => {
      newApiKeyInput.type = showNewApiKey.checked ? "text" : "password";
    });
    ...
    confirmBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        this.setStatus(
          "success",
          "API Key updated successfully! Testing connection..."
        );

        // Test the new API key immediately
        setTimeout(async () => {
          await this.testConnections();
          // If we're in the action section, try to refresh card data
          if (this.actionSection.style.display !== "none") {
    ...
    document.addEventListener("keydown", function escapeHandler(e) {
      if (e.key === "Escape") {
        if (document.getElementById("apiKeyOverwriteModal")) {
          document.body.removeChild(modal);
        }
  ...
  async updateApiKey(newApiKey) {
    console.log("🔍 updateApiKey called with key length:", newApiKey.length);

    try {
      // Update the input field
      this.apiKeyInput.value = newApiKey;
      ...
      const response = await this.sendMessage({
        ...
        config: {
          ...
          projectPath: this.projectPathInput.value.trim(),
          defaultBaseBranch: this.defaultBranchInput.value.trim(),
        },
      });
      ...
      if (response.success) {
        this.hideResults(); // Hide any previous error messages
      ...
      console.error("❌ Exception in updateApiKey:", error);
      throw error;
    }
  }
